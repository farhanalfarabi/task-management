...
const columns = [
  ...
  },
  {
    ...
    header: "Team Member",
    cell: ({ row }) => {
      const memberName = row.original.reviewer || row.original.teamMember || "N/A";
      // Generate avatar URL using placeholder service or use provided image
      const avatarUrl = row.original.avatar || row.original.image || 
        `https://api.dicebear.com/7.x/avataaars/svg?seed=${encodeURIComponent(memberName)}`;
      const fallbackUrl = `https://api.dicebear.com/7.x/avataaars/svg?seed=${encodeURIComponent(memberName)}`;
            ...
            <AvatarFallback className="bg-muted p-0">
              <img 
                ...
                onError={(e) => {
                  ...
                  e.target.src = `https://ui-avatars.com/api/?name=${encodeURIComponent(memberName)}&background=random&size=128`;
                }}
              />
            </AvatarFallback>
          </Avatar>
    ...
    cell: ({ row }) => (
      <div className="w-32">
        <Badge variant="outline" className="text-muted-foreground px-1.5">
          {row.original.type || row.original.role || "N/A"}
        </Badge>
      </div>
    ),
  },
    ...
    cell: ({ row }) => {
      const email = row.original.email || 
        (row.original.reviewer 
          ? `${row.original.reviewer.toLowerCase().replace(/\s+/g, '.')}@example.com`
          : "N/A");
  ...
  },
  {
    ...
    cell: ({ row }) => (
      <Badge variant="outline" className="text-muted-foreground px-1.5">
        {row.original.status === "Done" || row.original.status === "Active" ? (
        ...
        <div className="flex items-center gap-2 flex-1">
          <div className="relative flex-1 max-w-sm mr-4">
            ...
            <Input
              ...
              value={(table.getColumn("reviewer")?.getFilterValue() ?? "")}
              onChange={(event) =>
