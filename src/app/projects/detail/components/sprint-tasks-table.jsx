...
export function SprintTasksTable({ project, initialColumns }) {
  ...
  const statusColors = {
    planning: "bg-blue-100 text-blue-700 dark:bg-blue-900/30 dark:text-blue-400 border-blue-200 dark:border-blue-800",
    development: "bg-primary/10 text-primary dark:bg-primary/20 border-primary/20",
    deployment: "bg-purple-100 text-purple-700 dark:bg-purple-900/30 dark:text-purple-400 border-purple-200 dark:border-purple-800",
  };
  ...
  const getStatusFromColumn = (task, columns) => {
    ...
    for (const [columnKey, tasks] of Object.entries(columns || {})) {
      if (tasks.some(t => t.id === task.id)) {
        return columnKey; // Return column name directly (planning, development, deployment)
      }
    }
    return "planning";
  };
          ...
          <TableBody>
            {allTasks.length > 0 ? (
              allTasks.map((task, index) => {
                ...
                const isOverdue = dueDate && dueDate < new Date() && status !== "deployment";
